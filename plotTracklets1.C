void plotTracklets1(TString data = "data.root", TString mc = "mc.root")
{
...
if (!trd) return;
  ...
  trd->Draw("fPhi_reco>>hphi");
	...
	//heta->SetAxisRange(300, 700,"X");
	//hmul->Sumw2();
	//hmul_c->Scale(1.0/(1564),"width");
...
fDataRecoZvtx->Draw("p");
	...
	// t1->Draw("fDTheta_reco:fDPhi_reco>>hetaphi");
	// TH2F *hetaphi = (TH2F*)gPad->GetPrimitive("hetaphi");
	// hetaphi->GetXaxis()->SetTitle("#eta #phi");
	// hetaphi->GetYaxis()->SetTitle("Number of Events");
  ...
  hptp->GetXaxis()->SetTitle("pT (primary tracklets)");
    ...
    //
    // ts->Draw("fPt_seco>>hpts(100,0,20)");
  	// TH1F *hpts = (TH1F*)gPad->GetPrimitive("hpts");
    // hpts->GetXaxis()->SetTitle("pT");
  	// hpts->GetYaxis()->SetTitle("Secondary  tracklets");
...
//  tp->Draw("fPt_prim>>hptcut1(200,0,5)","fPt_seco<1.5");
...
//  hptcut1->Scale(1/500);
  ...
  TCanvas* cprof = new TCanvas("cprof", "cprof", 1100, 1100);
	cprof->Divide(2,2,0.01,0.01,0);
  // cprof_1->SetLogy();
  // cprof_2->SetLogy();
  // cprof_3->SetLogy();
  // cprof_4->SetLogy();
  ...
  cprof->cd(1);
	...
	tp->Draw("fPt_prim:fDPhi_prim>>hptdPhiProf","","PROF");


  TProfile *hptdPhiProf1 = (TProfile*)gPad->GetPrimitive("hptdPhiProf");
  hptdPhiProf1->GetXaxis()->SetTitle("#Delta#phi (Reconstructed)");
	hptdPhiProf1->GetYaxis()->SetTitle("pT (primary)");

  cprof->cd(2);

  tp->Draw("fPt_prim:fDTheta_prim>>hptdThetaProf","","PROF");


  TProfile *hptdThetaProf1 = (TProfile*)gPad->GetPrimitive("hptdThetaProf");
  hptdThetaProf1->GetXaxis()->SetTitle("#Delta#theta (Reconstructed)");
	hptdThetaProf1->GetYaxis()->SetTitle("pT (primary)");

cprof->cd(3);


tp->Draw("fPt_prim:fDPhi_prim>>hptdEtaCutProf(100,-0.01,0.01,200,0,2)","TMath::Abs(fDTheta_prim)<0.025","PROF");
//tp->Draw("fPt_prim:fDPhi_prim>>hptdEtaCutProf","TMath::Abs(fDTheta_prim)<0.025 && fDist_prim<1.3","PROF");


TProfile *hptdEtaCutProf1 = (TProfile*)gPad->GetPrimitive("hptdEtaCutProf");
hptdEtaCutProf1->GetXaxis()->SetTitle("#Delta#phi (Reconstructed) with Cut");
hptdEtaCutProf1->GetYaxis()->SetTitle("pT (primary)");


  cprof->cd(4);

  tp->Draw("fPt_prim:fDTheta_prim>>hptdThetaCutProf(100,-0.01,0.01,100,0,2)","TMath::Abs(fDPhi_prim)<0.080","PROF");
//  tp->Draw("fPt_prim:fDTheta_prim>>hptdThetaCutProf","TMath::Abs(fDPhi_prim)<0.080 && fDist_prim<1.3","PROF");


	TProfile *hptdThetaCutProf1 = (TProfile*)gPad->GetPrimitive("hptdThetaCutProf");
  hptdThetaCutProf1->GetXaxis()->SetTitle("#Delta#theta (Reconstructed) with Cut");
	hptdThetaCutProf1->GetYaxis()->SetTitle("pT (primary)");


	cprof->SaveAs("PtDeltaPhiProf.pdf");
